...
function Project() {
  ...
  const AddProject = async (userData) => {
    ...
    if (true) {
      ...
      } else if (tokenObject.tokenStatus === "google") {
        res = await fetch(`http://localhost:8000/user/project`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className={styled.signcontainer}>
        ...
        <div className={styled.prosection2} style={{ zIndex: 2 }}>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              dispatch(projectNameButton(userProject.project));
              ...
              <div className={styled.projectSubContainer}>
                <div className={styled.inputBox}>
                  <input
                    type="text"
                    id="project"
                    placeholder="eg... New App Launch project"
                    name="project"
                    defaultValue={userProject.project}
                    onChange={(e) => {
                      setUserProject({
                        ...userProject,
