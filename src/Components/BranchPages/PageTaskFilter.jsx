...
function PageTaskFilter() {
  const { projectname, tabid, taskName } = useParams();
  ...
  const [subtaskStatus,setSubtaskStatus]=useState({
    priority:{ value: "Low" ,image: getImageForOption1("Low")},
  ...
  const [selectedValues3, setSelectedValues3] = useState([]);
    ...
    useEffect(() => {
        setSelectedValues4([{ image: getImageForOption1(TaskInfoStateValue.taskInfoTask?.priority), value: TaskInfoStateValue.taskInfoTask?.priority }]);
        setSelectedValues3([{ image: getImageForOption(TaskInfoStateValue.taskInfoTask?.status), value: TaskInfoStateValue.taskInfoTask?.status }]);
    }, []);
   async function AddSubTask(userData){
    ...
    const tokenObject = JSON.parse(localStorage.getItem("userToken"));
      if (tokenObject.tokenStatus === "manual") {
        res = await fetch(`http://localhost:8000/user/mytask/add/subtask`, {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            tokenData: {
              ...
              kanbanData: {
                ...
                taskNameId:TaskInfoStateValue.taskInfoTask._id
              },
            },
          }),
        });
      ...
      if (data.status === 422) {
        ...
        // console.log("Added Sub Task in Mytask ",data.updatedUser.projects.find((ele,ind)=>{
    ...
    <>
      <div className={styled.section3} style={{position:"relative"}}>
        <div style={{ display: "flex" }}>
              ...
              return (
                <div className={styled.section3_Left}>
                  <Link
                    to={
                      ele.title === "Sub tasks"
                        ? `/home/${projectname}/${tabid}/mytask/${pageName[5]}/${taskName}/Sub`
                        : `/home/${projectname}/${tabid}/mytask/${pageName[5]}/${taskName}/${ele.title}`
                    }
                    className={
                      pageName[7] === "Sub"
                        ? "Sub tasks" === `${ele.title}`
                  ...
                  >
                    <div className={styled.creationPage}>
                      <div style={{ display: "flex", alignItems: "center" }}>
                        <div>{ele.title}  &nbsp;{ele.title=="Sub tasks"?<span style={{color:"#7f7575"}}>{myTaskStateValue
          .find(
            (ele) =>
              ele.title ==
              pageName[5].charAt(0).toUpperCase() + pageName[5].slice(1)
          ...
          ?.cards.find((ele, ind) => {
            if (ele._id == TaskInfoStateValue.taskInfoTask._id) return ele;
          })?.tasks[0]?.subtask.length}</span>:ele.title=="Issues"?<span style={{color:"#7f7575"}}>{myTaskStateValue
          .find(
            (ele) =>
              ele.title ==
              pageName[5].charAt(0).toUpperCase() + pageName[5].slice(1)
          ...
          ?.cards.find((ele, ind) => {
            if (ele._id == TaskInfoStateValue.taskInfoTask._id) return ele;
          })?.tasks[0]?.issues.length}</span>:""}</div>
                      </div>
                    </div>
                  </Link>
                </div>
        ...
        >
            <img src={pageName[7]==="Sub" ?Plus_icon:done} alt="sort" style={{width:"1.5rem",height:"1.5rem"}} />
            {pageName[7]==="Sub"?<span >Add subtask</span>:<span>Mark done</span> }
          </button>
         ...
         <button
          ...
          >
            <div>
              <img src={pageName[7]==="Sub"?sort:pen2} alt="sort" />
            </div>
            <div>
            { pageName[7]==="Sub"?  <span>Sort</span>:<span>Edit</span>}
            </div>
            </button>
        ...
        {TaskInfoStateValue.taskInfoToggValue? <div className={styled.taskInfo}>
        <div
          style={{
            display: "flex",
            alignItems: "center",
        ...
        <div className={styled.cutomizeSections} >
                ...
                <div
                    ...
                    >
                      ...
                      />
 <div>
 <span style={{color:"#ECEBEB"}}>{TaskInfoStateValue.taskInfoTask?.status}</span>
              ...
              <div className={styled.cutomizeSections}>
                ...
                <div
                    ...
                    >
                      ...
                      />
 <div>
 <span style={{color:"#ECEBEB"}}>{TaskInfoStateValue.taskInfoTask?.priority}</span>
              ...
              <div className={styled.cutomizeSections}>
                ...
                <span style={{color:"#ECEBEB"}}>{new Date(TaskInfoStateValue.taskInfoTask?.start).toLocaleString('default', { month: 'short', day: 'numeric' }).replace(/(\d{1,2})(st|nd|rd|th)/, '$1')} - {new Date(TaskInfoStateValue.taskInfoTask?.end).toLocaleString('default', { month: 'short', day: 'numeric' }).replace(/(\d{1,2})(st|nd|rd|th)/, '$1')}</span>
              </div>
              <div className={styled.cutomizeSections}>
                <div style={{ display: "flex", alignItems: "center" }}>
                  <div
