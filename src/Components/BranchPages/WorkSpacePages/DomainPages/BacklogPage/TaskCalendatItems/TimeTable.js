...
import styled from './taskCalendarStyle.module.css';
 ...
 const months = [
    ...
    for (let j = 1; j <= numDays; j++) {
      dayRow.push(
        <div key={j} style={{ ...ganttTimePeriod, outline: 'none' }}>
          <span style={ganttTimePeriodSpan}>{j},{dateDays[((j - 1) % 7) + 1]}</span>
        </div>
      );
  ...
  if (tasks) {
    tasks.forEach((task) => {
      ...
      for (let i = 0; i < numMonths; i++) {
          ...
          taskRow.push(
            <div
              ...
              style={{
                ...
                borderBottom:"1px solid #7f7575"
                // backgroundColor:
                //   dayOfTheWeek === 'S' ? 'var(--color-tertiary)' : 'green',
              }}
              data-task={task?._id}
            ...
            >
              {taskDurations.map((el, i) => {
                if (el?.parentId == task?._id && el?.start == formattedDate) {
                  return (
                    <div
                      key={`${i}-${el?._id}`}
                      draggable="true"
                      tabIndex="0"
                      onDragStart={() => handleDragStart(el?._id)}
                      style={{
                        ...taskDuration,
                        width: `calc(${dayDiff(
                          el?.start,
                        ...
                        opacity:
                          taskDurationElDraggedId === el?._id ? '0.5' : '1',
                      }}
                      onKeyDown={(e) => deleteTaskDuration(e, el?._id)}
                    ></div>
                  );
                }
              })}
        ...
        taskRows.push(
          <div key={`${i}-${task?._id}`} style={ganttTimePeriod1} className={styled.ganttTimePeriod21}>
            {taskRow}
          </div>
        );
  ...
  function deleteTaskDuration(e, id) {
    if (e.key === 'Delete' || e.key === 'Backspace') {
      ...
      const newTaskDurations = taskDurations.filter(
        (taskDuration) => taskDuration._id !== id
      );
  ...
  return (
    <div
      style={{ gridTemplateColumns: `repeat(${numMonths}, 1fr)` ,display:"grid",overflowX:"auto", }}
    >
      {/* {monthRows} */}
      {dayRows}
      {/* {weekRows} */}
